...
def save(result, dataset, result_folder, run):
    file_path = f'{result_folder}/Supervised_Results_no_Genus_new{dataset}.json'
...
def run(args):
    ...
    summary_file = "/home/m4safari/projects/def-lila-ab/m4safari/ext1prime/data/Extremophiles_GTDB.tsv"
    ...
    for k in range(1, args["max_k"] + 1):
        ...
        for name, algorithm in classifiers:
            ...
            #     x_test = test_Dataset.loc[:, list(range(n_features))].to_numpy()
            #     y_test = list(
            #         map(lambda x: unique_labels.index(x), test_Dataset.loc[:, ['random_label']].to_numpy().reshape(-1)))
                # print(y_test)
